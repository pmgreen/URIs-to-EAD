...
def _normalize_heading(heading):
	...
	"""
	collapsed = " ".join(heading.split()).replace(" -", "-").replace("- ", "-").replace(",--",", ").replace(".--","--")
	initial = re.search("\s[A-Z]\.$",collapsed)
	if (collapsed.endswith(".") and not initial) or collapsed.endswith(","):
...
# update_headings
#===============================================================================
def _update_headings(h, ctxt, shelf, annotate=False, verbose=False, mrx=False, log=False, ignore_cache=False):
				...
				if args.names:
					...
					for n in rec.get_fields(*names):
						for s in n.get_subfields(*namesubf):
							...
							mrx_subs.append(s)
						h = "--".join(mrx_subs)
						_update_headings(h, n, shelf, **options)
						mrx_subs = []
				if args.subjects:
					#=======================
					# SUBJECTS
					...
					for f in rec.get_fields(*subs):
						for s in f.get_subfields(*subsubf):
							...
							mrx_subs.append(s)
						h = "--".join(mrx_subs)
						_update_headings(h, f, shelf, **options)
						mrx_subs = []
